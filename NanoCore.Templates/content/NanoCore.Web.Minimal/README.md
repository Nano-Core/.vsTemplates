# NanoCore.Web.Minimal

> _Minimal Nano Web application (Razor/Blazor)._
...
## Summary
This application is a Nano Web app (Razor/Blazor) set up to be the entry point for external browser traffic: no data provider, no storage, no eventing — just the Nano Web
host, hardened security headers, Kubernetes ingress routing, Docker/Kubernetes deployment, and CI/CD.

The application itself is intentionally minimal and does not add any pages or components beyond the boilerplate root component (`App.razor`). Instead, it serves as a
baseline you build your actual application on top of.

`NanoWebApplication` extends `NanoApiApplication` with Razor Pages/Blazor support - see **[Nano Web Applications](https://github.com/Nano-Core/Nano.Library/blob/master/Nano.App.Web/README.md#nanoappweb)**
for what it adds on top of the Api application type.
...
* **`appsettings.json`** — a hardened `HttpPolicyHeaders` baseline (CSP, HSTS, Referrer Policy, Frame Options, Robots, Forwarded Headers), `ResponseCache`, `ResponseCompression`,
  and `Documentation` (Swagger). `Cors.AllowedOrigins` is empty by default — set the `CORS_ALLOWED_ORIGIN_0` repository variable to the origin(s) that should be allowed to call
  this application from a browser.
